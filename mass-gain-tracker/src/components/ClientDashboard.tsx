        ...
        <section>
          ...
          <div className="space-y-4">
            {plan.meals.map((meal: any) => {
              ...
              return (
                <div key={meal.id} onClick={() => { if (meal.recipe) toggleExpand(meal.id); }} className={`w-full text-left bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-5 flex flex-col gap-4 transition-all shadow-lg overflow-hidden relative ${meal.recipe ? 'cursor-pointer active:scale-[0.99]' : ''}`}>
                  {/* Glass highlight */}
                  ...
                  <div className="flex gap-4 items-start relative z-10">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {meal.recipe && <svg className={`w-5 h-5 text-slate-400 transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>}
                      </div>
